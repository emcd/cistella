//! Quadlet unit mechanics for the Podman isolator.
//!
//! Unit-content generation, install/start/stop/remove, scratch
//! handling, shared teardown, GC, and journal logs. The trait impl in
//! [`super::podman`] drives these; the `runtime` module re-exports
//! the unchanged paths.
...
/// Stops a unit and settles it (stop + wait for inactive/failed).
///
/// Under systemd this is `systemctl stop` plus the settle loop; without
/// a user manager it falls back to `podman stop --time <grace>`. The
/// grace bounds the no-systemd stop; under systemd the unit's own
/// `StopTimeout` governs (RunInit stops arrive in ~0.25 s).
///
/// # Errors
///
/// Returns `CistellaError::Runtime` on stop failure for a present unit.
pub(crate) fn stop_settle(container_name: &str, grace: Duration) -> Result<()> {
    ...
    let secs = grace.as_secs().max(2).to_string();
    let out = Command::new("podman")
        .args(["stop", "--time", &secs, container_name])
        .output()
...
/// Removes the unit file, reloads, and resets failure state (systemd path).
///
/// Absent files succeed without reloading: removing an absent unit is
/// the converged state, never an error.
///
/// # Errors
///
/// Returns `CistellaError::Runtime` on removal or reload failure.
pub(crate) fn remove_unit_file(container_name: &str, unit_name: &str) -> Result<()> {
    if !systemd_user_available() {
        return Ok(());
    }
    let service = format!("{container_name}.service");
...
pub fn teardown_inner(container_name: &str, session_id: &str) -> Result<()> {
    ...
    if systemd_user_available() {
        stop_settle(container_name, Duration::from_secs(10))?;
        remove_unit_file(container_name, &unit)?;
    } else {
        // Fallback for environments without systemd user instance (tests)
        stop_settle(container_name, Duration::from_secs(2))?;
    }
...
/// Reports container presence: exit 0 present, exit 1 absent.
///
/// The absence primitive for GC and inspect: any other exit is a typed
/// error that reaps nothing (a failed query never reads as absence).
pub(crate) fn container_exists(name: &str) -> Result<bool> {
    let exists = Command::new("podman")
