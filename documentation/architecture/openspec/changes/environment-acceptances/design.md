...
- **No deny on acceptances, by explicit operator override** (over Advisor's 0.1.1 retain-deny counsel, recorded in the proposal). Rationale: the exact list is itself the intent record. Diagnostics display names only; accepted values are stored in unit `Environment=` lines and visible to principals able to read or inspect them (no rescan into template error paths). Governing doctrine: exact-name declarations are deliberate operator intent — neither the deny lists nor future reviewers get a paternalist veto over what an operator does with a variable they named exactly (including forwarding or, later, rendering secrets). The only legitimate future questions about such attributes are semantics and mechanics, never permission.
