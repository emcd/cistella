...
- **Runtime: rootless Podman only for V1, driver abstracts.** For spike: `podman run -d --userns=keep-id` with `--label cistella-*`; for V1 driver: `run` generates a Quadlet `.container` unit and `systemctl --user start` it (systemd owns the container, not `podman run`). Quadlet adds `--rm --replace --cgroups=split --sdnotify=conmon -d` to `ExecStart` regardless of `Container` keys — containers are removed on `systemctl stop` by design; post-mortem is `journalctl --user -u <service>` not `podman logs`. `gc` reaps orphaned unit files and stray non-systemd containers (`cistella.*` labels). Quadlet user units, not `podman generate systemd`. `SuccessExitStatus=143` prevents `sleep infinity` SIGTERM from lingering in `failed`. AppArmor, not SELinux `:z`. Alternative Docker rejected until concrete need; VM isolation deferred. *Rationale:* spike is green on Podman; abstraction keeps `coders.toml` stable.
...
- **Mounts: allowlist-only triples `(host-source, container-target, mode)` with two-tier topology and explicit `HOME`.** The driver provides one distinguished writable session-home root (tmpfs or session scratch) at the explicit `HOME` (`container-home` field in profile schema, default `/home/cistella`), not a triple and not subject to peer-disjointness. Profile triples MUST be pairwise disjoint after canonicalization (no ancestor/descendant among triples) and MAY nest under the session-home root; mount order is session-home first, then triples by path depth. No `sibling masking` (deny-list) — under allowlist there is no parent to mask. Spike: `~/.config/opencode`, `~/.local/share/opencode`, worktree, `nb` RW, config RO. Alternative broad `XDG` RO parent rejected.
