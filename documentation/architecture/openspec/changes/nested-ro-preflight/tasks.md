...
- [ ] 1.1 Add `mount.rs` pure helpers: nearest-RO-ancestor selection over canonical targets plus container-suffix translation onto the ancestor host source, with unit tests (depth ordering, symlink-canonical inputs, type expectations).
- [ ] 1.2 Wire into `conduct_session` after validation, before scratch creation: existence + `is_dir` per chain component, typed error naming RO ancestor and missing host path, zero mutation on refusal (assert unit/scratch absence in tests).
